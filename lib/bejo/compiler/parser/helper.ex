defmodule Bejo.Compiler.Parser.Helper do
  import NimbleParsec
  ...
  def do_to_ast({[left, bin_op, right | rest], line}, :exp_bin_op)
      when bin_op in ["+", "-", "*", "/", "|", "&", "<", ">", "<=", ">=", "==", "!="] do
    new_left = {:call, {String.to_atom(bin_op), line}, [left, right], "bejo/kernel"}
  ...
  def do_to_ast({[unary_op, exp], line}, :unary_op)
      when unary_op in ["!", "-"] do
    {:call, {String.to_atom(unary_op), line}, [exp], "bejo/kernel"}
  end
