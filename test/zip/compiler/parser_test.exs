defmodule Zip.Compiler.ParserTest do
      ...
      assert {
               ...
               [
                 {:call, {:+, {1, 0, 22}},
                  [
                    {:integer, {1, 0, 2}, 11},
                    {:call, {:-, {1, 0, 7}}, [{:integer, {1, 0, 7}, 5}], "zip/kernel"}
                  ], "zip/kernel"},
                 {:call, {:/, {1, 0, 22}},
                  [
                    {:call, {:*, {1, 0, 22}},
                     [
                       {:integer, {1, 0, 12}, 10},
                       {:call, {:-, {1, 0, 17}}, [{:integer, {1, 0, 17}, 1}], "zip/kernel"}
                     ], "zip/kernel"},
                    {:call, {:-, {1, 0, 22}}, [{:integer, {1, 0, 22}, 2}], "zip/kernel"}
                  ], "zip/kernel"}
               ],
               "zip/kernel"
             } == TestParser.expression!(str)
               ...
               [
                 {:call, {:+, {1, 0, 22}},
                  [
                    {:integer, {1, 0, 2}, 10},
                    {:call, {:*, {1, 0, 12}},
                     [{:integer, {1, 0, 7}, 20}, {:integer, {1, 0, 12}, 30}], "zip/kernel"}
                  ], "zip/kernel"},
                 {:call, {:/, {1, 0, 22}},
                  [{:integer, {1, 0, 17}, 40}, {:integer, {1, 0, 22}, 50}], "zip/kernel"}
               ],
               "zip/kernel"
             } == TestParser.expression!(str)
    end
               ...
               [
                 {
                   ...
                   [
                     {:call, {:+, {1, 0, 8}},
                      [{:integer, {1, 0, 3}, 10}, {:integer, {1, 0, 8}, 20}], "zip/kernel"},
                     {:call, {:-, {1, 0, 20}},
                      [{:integer, {1, 0, 15}, 30}, {:integer, {1, 0, 20}, 40}], "zip/kernel"}
                   ],
                   "zip/kernel"
                 },
                 {:integer, {1, 0, 26}, 50}
    ...
    test "with args" do
      ...
      assert {:function, [position: {3, 40, 43}],
              {:foo,
               ...
               [
                 {:call, {:+, {2, 32, 39}},
                  [{:identifier, {2, 32, 35}, :x}, {:identifier, {2, 32, 39}, :y}], "zip/kernel"}
               ]}} ==
               TestParser.function_def!(str)
    end
  ...
  describe "match expression" do
    ...
    test "match exps can exist as a whole exp" do
      ...
      assert {:call, {:+, {1, 0, 11}},
              ...
              ], "zip/kernel"} == TestParser.expression!(str)
    end
  end
  ...
  describe "map" do
    ...
    test "parses map with complex expression" do
               ...
               [
                 {
                   {:list, {1, 0, 9},
                    [
                      {:call, {:+, {1, 0, 5}},
                       [{:integer, {1, 0, 3}, 1}, {:integer, {1, 0, 5}, 1}], "zip/kernel"},
                      {:integer, {1, 0, 8}, 2}
                    ]},
                   {:map, {1, 0, 31},
                    [
      ...
      assert {:map, {1, 0, 23},
              [
                {{:call, {:&, {1, 0, 13}},
                  [{:boolean, {1, 0, 5}, true}, {:boolean, {1, 0, 13}, false}], "zip/kernel"},
  ...
  describe "logical operators" do
    ...
    test "supports negation with more complex expressions" do
      {
        ...
        {:!, _},
        [
          {:call, {:|, _}, [{:boolean, _, true}, {:boolean, _, false}], "zip/kernel"}
        ],
        "zip/kernel"
    ...
    test "more complex expressions" do
      ...
      assert {
               ...
               [
                 {:boolean, _, true},
                 {:call, {:|, _}, [{:boolean, _, false}, {:boolean, _, true}], "zip/kernel"}
               ],
               "zip/kernel"
             } = TestParser.expression!(str)
    end
